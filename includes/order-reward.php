...
/* ---------------------------------------------------------
 * 3. Reward referrer on first completed order
 * --------------------------------------------------------- */
add_action('woocommerce_order_status_completed', function ($order_id) {
    ...
    if (get_post_meta($order_id, '_referral_processed', true)) return;

    $referrer_id = get_user_meta($user_id, '_woo_referrer', true);
    if (!$referrer_id || $referrer_id == $user_id) return;

    // Prevent multiple rewards for same user
    if (get_user_meta($user_id, '_referral_rewarded', true)) return;
    ...
    add_user_meta($referrer_id, '_referral_coupon', $coupon_code);
    ...
    update_post_meta($order_id, '_referral_processed', true);
    update_user_meta($user_id, '_referral_rewarded', true);
});
